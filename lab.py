...
class UserOptimizedTransformer(BaselineTransformer):
    """
    Submission-owned implementation.  Only this class is changed from the
    supplied benchmark template; baseline classes and benchmark utilities stay
    untouched.  Parameter names remain identical, so the original
    ``copy_model_weights`` works without modification.
    """

    def __init__(self, config: TransformerConfig) -> None:
        super().__init__(config)
        self.register_buffer(
            "_packed_qkv_weight", torch.empty(0), persistent=False
        )
        self.register_buffer("_packed_qkv_bias", torch.empty(0), persistent=False)

    def load_state_dict(self, state_dict, strict: bool = True, assign: bool = False):
        result = super().load_state_dict(state_dict, strict=strict, assign=assign)
        # The cache is rebuilt after the reference weights have been copied.
        weights = []
        biases = []
        for layer in self.layers:
    ...
    def _attention(
        ...
        # Causal + padding and reduced precision retain the reference order.
        strict_attention = x.dtype in (torch.float16, torch.bfloat16) or (
            causal and valid_token_mask is not None
        )
        if strict_attention:
            ...
            if self._packed_qkv_weight.numel() == 0:
                raise RuntimeError("packed QKV cache was not initialized")
            qkv = F.linear(
                x,
                self._packed_qkv_weight[layer_index],
                self._packed_qkv_bias[layer_index],
            )
            qkv = qkv.view(batch, seq_len, 3, heads, head_dim)
        ...
        if strict_attention:
            ...
            if valid_token_mask is not None:
                scores = scores.masked_fill(~valid_token_mask[:, None, None, :], float("-inf"))
            probs = torch.softmax(scores.float(), dim=-1).to(dtype=x.dtype)
            context = torch.matmul(probs, v)
        else:
            if valid_token_mask is None:
                context = F.scaled_dot_product_attention(q, k, v, dropout_p=0.0, is_causal=causal)
            else:
                mask = valid_token_mask[:, None, None, :]
                context = F.scaled_dot_product_attention(q, k, v, attn_mask=mask, dropout_p=0.0, is_causal=False)

        context = context.transpose(1, 2).contiguous().view(batch, seq_len, d_model)
        output = layer.attention.out_proj(context)
        if valid_token_mask is not None:
            output = output.masked_fill(~valid_token_mask[..., None], 0)
        return output
    ...
    def _fast_forward(
        ...
        try:
            ...
            # Keep the class importable both as ``python tiktok/lab.py``
            # and as ``import tiktok.lab`` from the project root.
            from tiktok.transformer_kernels import (
                fused_residual_layer_norm,
                fused_residual_layer_norm_inplace,
            )

        # A one-row Triton launch has a fixed scheduling cost.  For tiny
        # activations the vendor LayerNorm kernel is faster than launching our
        ...
        for layer_index, layer in enumerate(self.layers):
            attention_output = self._attention(
                layer, layer_index, normalized, valid_token_mask
            )
            if not use_fused_ln:
                ...
                x, normalized_for_ffn = fused_residual_layer_norm(
                    x, attention_output, layer.norm2.weight, layer.norm2.bias,
                    valid_token_mask, layer.norm2.eps, False,
                )
            else:
                normalized_for_ffn = fused_residual_layer_norm_inplace(
                    x, attention_output, layer.norm2.weight, layer.norm2.bias,
                    valid_token_mask, layer.norm2.eps, False,
                )
            ffn_output = layer.ffn_out(
                F.gelu(layer.ffn_in(normalized_for_ffn), approximate="none")
            )
            is_last = layer_index + 1 == len(self.layers)
            next_norm = self.final_norm if is_last else self.layers[layer_index + 1].norm1
            if not use_fused_ln:
                x = x + ffn_output
                if valid_token_mask is not None:
                    x = x.masked_fill(~valid_token_mask[..., None], 0)
                normalized = next_norm(x)
            else:
                normalized = fused_residual_layer_norm_inplace(
                    x, ffn_output, next_norm.weight, next_norm.bias,
                    valid_token_mask, next_norm.eps, is_last,
                )
        return normalized
    ...
    def _strict_forward(
        ...
        """Keep the supplied operation order for CPU/training/low precision."""
    ...
    def forward(
        ...
        if (
            ...
            # The reference applies both a causal and a padding mask to the
            # materialized score matrix.  Keeping that exact path avoids
            # backend-dependent mask ordering and is preferable to a slower
            # partial optimization for this uncommon combination.
            or (self.config.causal and valid_token_mask is not None)
        ):
            return self._strict_forward(x, valid_token_mask)
        return self._fast_forward(x, valid_token_mask)
        # ============================================================
